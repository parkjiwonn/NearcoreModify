...
pub struct EpochManager {
    ...
    /// 현재 에포크에 대한 통계를 보관하는 aggregator
    /// 데이터는 마지막 최종 블록까지 동기화 된다.
    /// 정보는 [`Self::update_epoch_info_aggregator_upto_final`] method를 통해 업데이트 된다.
    /// 마지막 블록까지의 통계 사용하기 위해 [`Self::get_epoch_info_aggregator_upto_last`] method 사용한다.
...
impl EpochManager {
    ...
    /// 쫓아낼 검증인을 계산할 때 일부 검증인을 먼저 면제해 총 지분이 임계값을 초과하지 않도록 한다.
    /// 네트워크가 불안정할 경우 너무 많은 검증인을 쫓아내지 않도록 하기 위함이다.
    /// 또한 면제된 검증인이 지난번 에포크에서 쫓겨나지 않았는지 확인한다.
    /// 따라서 이 에포크 이후에도 검증자로 남아있을 수 있도록 보장한다.
