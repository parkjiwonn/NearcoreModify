...
/// 제공된 지분과 필요한 좌석 수가 주어졌을 때 좌석당 지분 임계값을 구한다.
...
/// 현재 좌석 배정 및 제안을 기반으로 새 좌석 배정을 계산한다.
pub fn proposals_to_epoch_info(
    epoch_config: &EpochConfig,
    rng_seed: RngSeed,
    prev_epoch_info: &EpochInfo, /// 이전 epoch에 대한 정보
    proposals: Vec<ValidatorStake>, /// 제안된 validator의 스테이크를 나타내는 ValidatorStake 구조체
    validator_kickout: HashMap<AccountId, ValidatorKickoutReason>,
    validator_reward: HashMap<AccountId, Balance>, /// 검사자들 보상
    minted_amount: Balance,
    next_version: ProtocolVersion,
    last_epoch_version: ProtocolVersion,
    /// 함수는 Result<EpochInfo, EpochError> 를 반환한다.
) -> Result<EpochInfo, EpochError> {
    /// last_epoch_version이 stable feature & AliasValidatorSelectionAlgorithm 을 지원하는지 확인.
    if checked_feature!("stable", AliasValidatorSelectionAlgorithm, last_epoch_version) {
        ...
        /// 지원하지 않는 경우  old_validator_selection::proposals_to_epoch_info를 호출해 이전 방식으로 좌석배정 계산한다.
